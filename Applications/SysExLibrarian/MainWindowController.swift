...
class MainWindowController: GeneralWindowController {
    ...
    func addReadMessagesToLibrary() {
        ...
        do {
            ...
            let informativeTextPart1 = String(localized: "The file could not be created.", comment: "message of alert when recording to a new file fails")
            ...
            if let newName = object as? String {
                if entry.renameFile(newName) {
                    ...
                    if let window {
                        ...
                        alert.informativeText = String(localized: "The file for this item could not be renamed.", comment: "message of alert when renaming a file fails")
                        alert.beginSheetModal(for: window, completionHandler: nil)
                    }
                }
            }
...
extension MainWindowController /* Private */ {
    ...
    private func synchronizeDestinationToolbarMenu(destinationGroups: [[OutputStreamDestination]], currentDestination: OutputStreamDestination?) {
        ...
        let selectedDestinationTitle = titleForDestination(currentDestination) ?? String(localized: "None", comment: "none")

        let topTitle = String(localized: "Destination", comment: "title of destination toolbar item") + ": " + selectedDestinationTitle
